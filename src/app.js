...
function applyPropertyDetails() {
  ...
  const generalGrid = document.querySelector('.general-grid');
  if (!property || !summary) return;
  if (generalGrid && property.sector && !generalGrid.querySelector('.sector-detail')) generalGrid.insertAdjacentHTML('beforeend', `<span class="sector-detail"><small>Sector</small><b>${property.sector}</b></span>`);
...
function whatsappUrl(property) {
  const message = `Hola ${contentSettings.agent_name}, estoy interesado en la propiedad ${property.title} (código ${propertyCode(property)}). ¿Sigue disponible?`;
