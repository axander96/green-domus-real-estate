...
function headerTemplate() {
  return `<header class="site-header"><a class="brand" href="#"><img src="/Favicon%20o%20logo/Dise%C3%B1o%20sin%20t%C3%ADtulo.svg" alt=""/><span>GREEN DOMUS<small>REAL ESTATE</small></span></a><nav><a href="#properties">Propiedades</a><a href="#about">Nosotros</a><a href="#contact">Contacto</a></nav><button class="outline-button admin-trigger">${icon('menu')} Panel admin</button><button class="mobile-menu">${icon('menu')}</button></header>`;
}
...
function detailTemplate(property) {
  ...
  return `<div class="page-shell">${headerTemplate()}<main class="detail-page"><a href="#all" class="back-link">${icon('arrow')} Volver a propiedades</a><div class="detail-layout"><div class="detail-left"><div class="detail-gallery"><img src="${gallery[0]}" alt="${property.title}"/><div class="gallery-note">${property.tag}<span>Código ${propertyCode(property)}</span></div></div><div class="detail-thumbnails">${gallery.map((image, index) => `<button class="detail-thumbnail ${index === 0 ? 'active' : ''}" data-gallery-image="${image}" aria-label="Ver imagen ${index + 1}"><img src="${image}" alt=""/></button>`).join('')}</div><section class="detail-description"><div><p class="eyebrow dark">SOBRE ESTA PROPIEDAD</p><h2>Un lugar para<br/><em>vivirlo todo.</em></h2></div><div><p>${property.description}</p><p>Diseñada para quienes valoran la ubicación, la calidad y la posibilidad de hacer suyo cada rincón. Conoce todos los detalles conversando con nuestro equipo.</p></div></section></div><div class="detail-right"><div class="detail-summary"><div class="property-meta"><span>${property.operation}</span><span>${property.type}</span></div><h1>${property.title}</h1><p class="location">${icon('pin')}${property.location}</p><strong class="detail-price">${formatPrice(property.price, property.operation)}</strong><div class="detail-specs"><span>${icon('area')}<b>${property.area} m²</b>Área total</span>${property.beds ? `<span>${icon('bed')}<b>${property.beds}</b>Habitaciones</span>` : ''}${property.baths ? `<span>${icon('bath')}<b>${property.baths}</b>Baños</span>` : ''}</div></div><aside class="agent-card"><div class="agent-heading"><div class="agent-avatar">AS</div><div><h3>Asdrúbal Salas</h3><p>Agente de bienes raíces</p></div></div><div class="agent-contact"><a href="tel:+18096711120">${icon('phone')} (809) 671-1120</a><a href="tel:+18296847760">${icon('phone')} (829) 684-7760</a><a href="mailto:greendomusrealestate@gmail.com">${icon('mail')} greendomusrealestate@gmail.com</a></div><textarea readonly>Hola Asdrúbal, tengo un cliente interesado en esta propiedad (código ${propertyCode(property)}). ¿Sigue disponible?</textarea><a class="whatsapp-button" href="${whatsappUrl(property)}" target="_blank" rel="noreferrer">${icon('whatsapp')} Enviar este mensaje ${icon('arrow')}</a></aside></div></div></main>${adminModal()}</div>`;
}
